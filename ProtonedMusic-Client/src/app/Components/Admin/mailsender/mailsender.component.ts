...
export class MailsenderComponent implements OnInit {
  ...
  selected: string[] = [];
  date: any = new Date();
  footerContent = '<br><br><br><br>' + '<footer><a style"display: flex; align-content: center;" href="https://protonedmusic.com/#/upcoming">Unsubscribe</a></footer>';
    ...
    if (this.sendToAll) {
      this.users.forEach((user) => {
        if (user.addonRoles === 'Newsletter' && user.email) {
          ...
          this.mail.body = this.mail.body + this.footerContent;
